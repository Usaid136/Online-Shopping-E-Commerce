...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT order_status FROM orders_info WHERE order_id = ?";
    ...
    if ($stmt->fetch()) {
        echo "<h3 class='text-center'>Your order of order number <span style='color: rgba(255, 135, 0, 0.74);'>" . $order_id ."</span>  has been <span style='color: rgba(255, 135, 0, 0.74);'>" . $order_status . "</span>.<br>Thanks for shopping with us!</h3>";
    } else {
        echo "<h3>Order not found. Please check the order ID.</h3>";
    }
